...
function say(player, values) {
  ...
  return tryDownloadTTS(text, language)
    .then((result) => {
      const base = settings.announcementBaseUrl
        ? settings.announcementBaseUrl.replace(/\/$/, '')
        : `http://${system.localEndpoint}:${port}`;
      return singlePlayerAnnouncement(player, `${base}${result.uri}`, announceVolume, result.duration);
    });
}
