...
function simplifyPlayer(player) {
  return {
    ...
    coordinator: player.coordinator ? player.coordinator.uuid : null,
...
function simplifyZones(zones) {
  return zones
    .filter((zone) => zone && zone.coordinator && zone.coordinator.uuid)
    .map((zone) => {
      return {
        uuid: zone.uuid,
        coordinator: simplifyPlayer(zone.coordinator),
        members: (zone.members || []).filter((m) => m && m.uuid).map(simplifyPlayer)
      };
    });
};
