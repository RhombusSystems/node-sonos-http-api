...
function refreshZone(zone) {
  ...
  const coordCalls = Promise.all([
    ...
    soapRequest(baseUrl, AV, AV_URN, 'GetTransportInfo').then(body => {
      const state = tag(body, 'CurrentTransportState');
      if (state) coord.state.playbackState = state;
    ...
    soapRequest(baseUrl, AV, AV_URN, 'GetPositionInfo').then(body => {
      ...
      if (trackStr) {
        ...
        if (Number.isFinite(n)) coord.state.trackNo = n;
      }
      if (relTime) coord.state.elapsedTime = parseTimeStr(relTime);
      coord.state.stateTime = Date.now();
    ...
    soapRequest(baseUrl, AV, AV_URN, 'GetTransportSettings').then(body => {
      ...
      if (pm) {
        if (!coord.state.playMode) coord.state.playMode = {};
        coord.state.playMode.repeat = parseRepeatMode(pm);
      }
  ...
  const memberCalls = (zone.members || []).map((member) => {
    if (!member || !member.baseUrl) return Promise.resolve();
...
module.exports = { refreshSystemState };
