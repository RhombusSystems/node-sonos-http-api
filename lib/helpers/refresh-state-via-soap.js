...
function soapRequest(playerBaseUrl, controlPath, serviceUrn, action, innerXml = '', timeoutMs = SOAP_TIMEOUT_MS) {
...
// Reachability probe for the announcement helpers: one GetTransportInfo per
// player with a short dedicated timeout. applyPreset addresses every listed
// player directly, so a single unreachable speaker (powered off, rebooting,
// dropped off the VLAN) rejects the whole group call with EHOSTUNREACH and
// the entire announcement fails — 2026-07-28 Land Park Rear went dark
// mid-day and every /clipsome aborted, silencing the two healthy rooms too.
// Callers run this in parallel with refreshSystemState (both are read-only
// SOAP) and drop unreachable players before building the announce preset.
//
// Resolves to a Set of reachable player uuids; never rejects.
const PROBE_TIMEOUT_MS = 2000;

function probeReachable(players) {
  return Promise.all((players || []).map((p) => {
    if (!p || !p.baseUrl) return Promise.resolve(null);
    return soapRequest(p.baseUrl, AV, AV_URN, 'GetTransportInfo', '', PROBE_TIMEOUT_MS)
      .then(() => p.uuid)
      .catch((err) => {
        logger.warn(`probe: ${p.roomName} unreachable (${err.message})`);
        return null;
      });
  })).then((uuids) => new Set(uuids.filter(Boolean)));
}

module.exports = { refreshSystemState, getTransportState, getCurrentVolume, probeReachable };
