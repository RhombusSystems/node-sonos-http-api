...
function subsetAnnouncement(system, roomNames, uri, volume, duration) {
  ...
  // A room missing from the discovered topology (speaker offline, dropped by
  // the SOAP poll) must not abort the whole announcement — skip it and play
  // on whatever is still discovered, mirroring the reachability-probe
  // degradation below. Only fail when nothing resolves at all.
  const resolved = [];
  const unknownRooms = [];
  for (const name of roomNames) {
    const player = findPlayer(system, name);
    if (player) resolved.push(player);
    else unknownRooms.push(name);
  }
  if (resolved.length === 0) {
    return Promise.reject(new Error(`subsetAnnouncement: no requested rooms found in topology: ${roomNames.join(', ')}`));
  }
  if (unknownRooms.length > 0) {
    logger.warn(`subsetAnnouncement: skipping rooms not in topology [${unknownRooms}] — proceeding with [${resolved.map((p) => p.roomName)}]`);
  }
  ...
  return Promise.all([
    ...
    const skippedRooms = unknownRooms.concat(dead.map((p) => p.roomName));
    return _subsetAnnouncementCore(system, live, new Set(dead.map((p) => p.uuid)), uri, volume, duration)
      .then(() => ({
        status: 'success',
        playedRooms: live.map((p) => p.roomName),
        skippedRooms,
      }));
  });
}
