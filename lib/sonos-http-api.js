...
function HttpAPI(discovery, settings) {
  ...
  function invokeWebhook(type, data) {
    ...
    // Serialization guard: Player.toJSON reads this.coordinator.uuid, and a
    // topology churn can leave member.coordinator undefined (playerCache miss
    // in SonosSystem.topologyChange) — the resulting TypeError would otherwise
    // throw synchronously into the topology-change emit path and stall
    // whatever triggered it (observed dragging an announcement restore +120s).
    let jsonBody;
    try {
      ...
      logger.warn(`invokeWebhook: could not serialize ${type} event (${err.message}); skipping`);
      return;
    }
